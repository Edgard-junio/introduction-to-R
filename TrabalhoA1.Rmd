---
title: "Trabalho de Introdução ao R Aplicado em Ciência de Dados"
author: "Edgard Junio"
date: "2025-09-19"
...
# Forçar atualização do ggplot2 e patchwork (evita conflito de versão caso exista)
...
Na primeira impressão o Brasil passou por um período de desmatamento altíssimo no início dos anos 2000, seguido por uma queda acentuada e expressiva até cerca de 2012, e uma posterior fase de estabilização em um patamar mais baixo, mas com uma preocupante tendência de alta nos anos mais recentes.
...
Para contextualizar a tendência de desmatamento, analisamos também a evolução do PIB no mesmo período. Como uma plotagem individual por município seria visualmente poluída optamos por uma abordagem agregada, calculando a média do PIB tornando interessante também, plotar o avanço do desmatamento médio ano a ano. A criação desses dois gráficos nos permite comparar as trajetórias da atividade econômica e do desmatamento, buscando identificar padrões de correlação visual.
...
O dataframe referente a união dos dados possui as seguintes variáveis (apenas as que foram usadas):
...
Todos os pacotes usados serão verificados se estão instalados na sua máquina, logo após a verificação é feita a intalação dos pacotes que faltam.
...
* Prompt 2 : Me ajude a criar um gráfico de dispersão.
